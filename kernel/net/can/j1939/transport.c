...
static struct
j1939_session *j1939_session_get_passive(struct j1939_priv *priv,
					 struct j1939_addr *addr)
{
	struct j1939_session *session;
	...
	list_for_each_entry(session, &priv->active_session_list,
			    active_session_list_entry) {
		j1939_session_get(session);
		if (session->skcb.addr.sa == addr->da &&
		    session->skcb.addr.da == addr->sa)
			return session;
		j1939_session_put(session);
	}
...
static int j1939_session_tx_cts(struct j1939_session *session)
{
	...
	if (j1939_sk_recv_dst_passive(priv, &session->skcb))
		return 0;
		  ...
		  bool transmitter)
{
	...
	if (j1939_sk_recv_dst_passive(priv, skcb)) {
		session = j1939_session_get_passive(priv, &skcb->addr);
		if (session) {
			j1939_session_timers_cancel(session);
			j1939_session_completed(session);
