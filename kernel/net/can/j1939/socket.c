...
static void j1939_jsk_add(struct j1939_priv *priv, struct j1939_sock *jsk)
{
	...
	kref_get(&priv->promisc_kref);
	spin_unlock_bh(&priv->j1939_socks_lock);
}

static void __j1939_promisc_release(struct kref *kref)
{}
...
static void j1939_jsk_del(struct j1939_priv *priv, struct j1939_sock *jsk)
{
	...
	kref_put(&priv->promisc_kref, __j1939_promisc_release);
	spin_unlock_bh(&priv->j1939_socks_lock);
...
bool j1939_sk_recv_dst_passive(struct j1939_priv *priv,
			       struct j1939_sk_buff_cb *skcb)
{
	struct j1939_sock *jsk;
	bool match = false;
