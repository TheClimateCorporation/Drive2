				   ...
				   uint sig_len, int node)
{
	...
	cell = fdt_getprop(blob, node, "rsa,num-bits", &length);
	if (!cell || length != sizeof(*cell))
		prop.num_bits = 0;
	else
		prop.num_bits = fdt32_to_cpu(*cell);

	cell = fdt_getprop(blob, node, "rsa,n0-inverse", &length);
	if (!cell || length != sizeof(*cell))
		prop.n0inv = 0;
	else
		prop.n0inv = fdt32_to_cpu(*cell);
