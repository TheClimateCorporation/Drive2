...
#define BOOTENV_DEV_FASTBOOT(devtypeu, devtypel, instance) \
	"bootcmd_fastboot=" BOOTENV_RUN_FASTBOOT_START "\0"
#define BOOTENV_DEV_NAME_FASTBOOT(devtypeu, devtypel, instance) \
	"fastboot "
#define BOOTENV_RUN_FASTBOOT_START "run boot_fastboot_start"
#define BOOTENV_SHARED_FASTBOOT \
	"boot_fastboot_start=" \
		"if test -e ${fastboot_led}; then " \
			"for each in ${fastboot_led}; do "\
				"led ${each} on; " \
			"done; " \
		"fi;" \
		"if test -e ${fastboot_led_off}; then " \
			"for each in ${fastboot_led_off}; do "\
				"led ${each} off; " \
			"done; " \
		"fi;" \
		"fastboot 0\0"
#else
#define BOOTENV_RUN_FASTBOOT_START
#define BOOTENV_SHARED_FASTBOOT
#define BOOTENV_DEV_FASTBOOT \
	BOOT_TARGET_DEVICES_references_FASTBOOT_without_CONFIG_CMD_FASTBOOT
#define BOOTENV_DEV_NAME_FASTBOOT \
	BOOT_TARGET_DEVICES_references_FASTBOOT_without_CONFIG_CMD_FASTBOOT
#endif
