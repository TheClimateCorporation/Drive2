...
static int do_mmc_block_test(struct cmd_tbl *cmdtp, int flag,
                             int argc, char * const argv[])
{
	...
	char feedback[FEEDBACK_SIZE];
        ...
        if (0 == mmc_block_test(mmc, feedback, FEEDBACK_SIZE))
        {
                return CMD_RET_SUCCESS;
        }
...
static int do_mmc_speed_test(struct cmd_tbl *cmdtp, int flag,
		             int argc, char * const argv[])
{
	struct mmc *mmc;
	char feedback[120];
        unsigned int hz = 400000;

	if (argc == 2)
		hz = simple_strtoul(argv[1], NULL, 10);
	else {
		printf("Usage: mmc spped_test hz\n");
		return CMD_RET_FAILURE;
	}
	...
	if (0 == mmc_speed_test(mmc, hz, feedback, FEEDBACK_SIZE))
	{
		return CMD_RET_SUCCESS;
	}
...
static int do_mmc_wp(struct cmd_tbl *cmdtp, int flag,
		     int argc, char * const argv[])
{
	int dev, ret;
	char *cmd;
	...
	if (!strcmp(cmd, "set")) {
		if (argc == 5) {
			if (!strcmp(argv[4], "none"))
				type = MMC_WP_NONE;
			else if (!strcmp(argv[4], "temporary"))
				type = MMC_WP_TEMPORARY;
			else if (!strcmp(argv[4], "poweron"))
				type = MMC_WP_POWERON;
			else if (!strcmp(argv[4], "permanent"))
				type = MMC_WP_PERMANENT;
			else
				return CMD_RET_USAGE;
		} else {
			return CMD_RET_USAGE;
		...
		return mmc_set_block_wp_status(mmc, addr,
						mmc->hc_wp_grp_size, type);
	}

	ret = mmc_get_block_wp_status(mmc, addr, &type);
	if (ret == 0) {
		switch (type) {
		case MMC_WP_NONE:
			puts("none\n");
			break;
		case MMC_WP_TEMPORARY:
			puts("temporary\n");
			break;
		case MMC_WP_POWERON:
			puts("poweron\n");
...
static struct cmd_tbl cmd_mmc[] = {
	...
	U_BOOT_CMD_MKENT(block_test, 2, 1, do_mmc_block_test, "", ""),
	U_BOOT_CMD_MKENT(speed_test, 2, 1, do_mmc_speed_test, "", ""),
#if CONFIG_IS_ENABLED(MMC_HW_PARTITIONING)
...
#endif
#if CONFIG_IS_ENABLED(MMC_WRITE_PROTECT)
	U_BOOT_CMD_MKENT(wp, 5, 0, do_mmc_wp, "", ""),
#endif
};
...
U_BOOT_CMD(
	...
	"mmc block_test - write/read block test\n"
	"mmc speed_test [hz]- test MMC interface at a given speed\n"
#if CONFIG_IS_ENABLED(MMC_HW_PARTITIONING)
...
#if CONFIG_IS_ENABLED(MMC_WRITE_PROTECT)
	"mmc wp dev get addr\n"
	" - Get write-protect group mode for group starting at addr\n"
	"mmc wp dev set addr {none|temmporary|poweron|permanent}\n"
	" - Set write-protect group to mode for group starting at addr\n"
	"    WARNING: permanent write-protect cannot be undone.\n"
