...
static struct mux_name muxtrl_name[] = {
	{ .regval = 0x00, .name = "DISABLE" },
	{ .regval = 0x01, .name = "VBAT" },
	{ .regval = 0x02, .name = "VSYS" },
	{ .regval = 0x03, .name = "VTS" },
	{ .regval = 0x04, .name = "VICHARGE" },
	{ .regval = 0x05, .name = "MUX_IN" },
	{ .regval = 0x06, .name = "DISABLE_6" },
	{ .regval = 0x07, .name = "DISABLE_7" },
};
...
static ssize_t tps65217_mux_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	...
	ret = tps65217_reg_read(tps, TPS65217_REG_MUXCTRL, &val);
	if (ret) {
		dev_err(tps->dev, "Read from MUXCTRL reg failed\n");
		return -EIO;
	}

	for (i = 0; i < ARRAY_SIZE(muxtrl_name); i++) {
		if (val == muxtrl_name[i].regval) {
			return snprintf(buf, PAGE_SIZE, "%s\n",
					muxtrl_name[i].name);
		}
	}

	dev_err(tps->dev, "Invalid MUXCTRL value: 0x%x\n", val);
	return snprintf(buf, PAGE_SIZE, "invalid\n");
}

static ssize_t tps65217_mux_store(struct device *dev,
				  ...
				  const char *buf, size_t count)
{
	...
	for (i = 0; i < ARRAY_SIZE(muxtrl_name); i++) {
		if (!strncmp(buf, muxtrl_name[i].name, len)) {
			ret = tps65217_reg_write(tps, TPS65217_REG_MUXCTRL,
						 muxtrl_name[i].regval,
						 TPS65217_PROTECT_NONE);
			if (ret) {
				dev_err(tps->dev, "Error writing to MUXCTRL\n");
				return ret;
			}
			return count;
		}
...
static DEVICE_ATTR(muxctrl, 0664,
	tps65217_mux_show, tps65217_mux_store);

static struct attribute *tps65217_mux_attrs[] = {
	&dev_attr_muxctrl.attr,
	NULL
};

static struct attribute_group tps65217_mux_gr = {
	.name = "tps65217-mux",
	.attrs = tps65217_mux_attrs
};
...
static int tps65217_probe(struct i2c_client *client)
{
	...
	ret = sysfs_create_group(&tps->dev->kobj, &tps65217_mux_gr);
	if (ret < 0) {
		dev_err(tps->dev, "sysfs device create file failed\n");
		return ret;
	}
...
static int tps65217_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&tps->dev->kobj, &tps65217_mux_gr);
