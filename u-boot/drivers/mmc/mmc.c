...
#if CONFIG_IS_ENABLED(MMC_WRITE_PROTECT)
int mmc_get_block_wp_status(struct mmc *mmc, lbaint_t addr, enum mmc_write_protect *status)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
	char val[8]; /* hold the 64-bit response */
	int err;
	...
	if (err == 0) {
		/* CMD31 returns 64 bit value; only want the last 2 bits */
		*status = (enum mmc_write_protect)(val[7] & 0x3);
	}

	return err;
}

int mmc_set_block_wp_status(struct mmc *mmc, lbaint_t start, lbaint_t blkcnt, enum mmc_write_protect status)
{
	int err;
	uint8_t reg_user_wp;
	lbaint_t blocknum;
	...
	if (ext_csd[EXT_CSD_REV] < 5) {
		printf("Write protect not supported\n");
		return -EINVAL;
	}

	if ((status == MMC_WP_POWERON) &&
	    (ext_csd[EXT_CSD_USER_WP] & EXT_CSD_US_PWR_WP_DIS)) {
		printf("Power on WP disabled\n");
		return -EINVAL;
	} else if ((status == MMC_WP_PERMANENT) &&
		   (ext_csd[EXT_CSD_USER_WP] & EXT_CSD_US_PERM_WP_DIS)) {
		printf("Permanent WP disabled\n");
		return -EINVAL;
	}

	if ((start % mmc->hc_wp_grp_size) || (blkcnt % mmc->hc_wp_grp_size)) {
		printf("Error: start and blkcnt must be a multiple of ");
		printf("write protect group size (%d)\n", mmc->hc_wp_grp_size);
		return -EINVAL;
	}

	if (status != MMC_WP_NONE) {
		reg_user_wp = ext_csd[EXT_CSD_USER_WP];
		reg_user_wp &= ~(EXT_CSD_US_PWR_WP_DIS |  \
				 EXT_CSD_US_PERM_WP_DIS | \
				 EXT_CSD_US_PERM_WP_EN |  \
				 EXT_CSD_US_PWR_WP_EN);
		switch (status) {
		case MMC_WP_POWERON:
			reg_user_wp |= EXT_CSD_US_PWR_WP_EN;
			break;
		case MMC_WP_PERMANENT:
			reg_user_wp |= EXT_CSD_US_PERM_WP_EN;
			break;
		default:
			break;
		}
		...
		if (reg_user_wp != ext_csd[EXT_CSD_USER_WP]) {
			err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
					 EXT_CSD_USER_WP, reg_user_wp);
			if (err) {
	...
	cmd.resp_type = MMC_RSP_R1b;
	for (blocknum = 0; blocknum < blkcnt; blocknum += mmc->hc_wp_grp_size) {
		cmd.cmdidx = (status == MMC_WP_NONE) ? \
			     MMC_CMD_CLEAR_WRITE_PROT : MMC_CMD_SET_WRITE_PROT;
		cmd.cmdarg = start + blocknum;
		...
		err = mmc_send_status(mmc, &result);
		if (err)
			return err;
	}
	...
	if (status != MMC_WP_NONE) {
		err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
				 EXT_CSD_USER_WP, ext_csd[EXT_CSD_USER_WP]);
		if (err) {
...
#endif
#if CONFIG_IS_ENABLED(MMC_HW_PARTITIONING) || CONFIG_IS_ENABLED(MMC_WRITE_PROTECT)
...
#define SPEED_TEST_BEGIN         0xA0000 // Beginning of root-b partition
#define BLOCK_TEST_BEGIN         0x11000 // Beginning of u-boot-b partition
#define NUM_TEST_BLOCKS          256     // Number of blocks to test per batch
#define PRINT_INTERVAL           50000   // Block interval to print an update

void print_diffs(int address, uint8_t a, uint8_t b)
{
	int i;

	printf("0x%04x: 0x%02x 0x%02x : ", address, a, b);
	for (i = 7; i >= 0; i--)
	{
		if ( ((1<<i)&a) == ((1<<i)&b))
		{
...
int mmc_block_test(struct mmc *mmc, char *feedback, int feedback_size)
{
	int ret, i;
	int endblock;
	uint8_t data[MMC_MAX_BLOCK_LEN*NUM_TEST_BLOCKS], verify[MMC_MAX_BLOCK_LEN*NUM_TEST_BLOCKS];
	uint8_t value = 0xA5;
	...
	for (i = 0; i < MMC_MAX_BLOCK_LEN*NUM_TEST_BLOCKS; i++) {
		data[i] = value;
                /* circle shift left */
		value = (value << 1) | (value >> (7));
	}

	endblock = (mmc->capacity_user / MMC_MAX_BLOCK_LEN) - BLOCK_TEST_BEGIN;

	for (i = BLOCK_TEST_BEGIN; i < endblock; i = i + NUM_TEST_BLOCKS) {
		ret = blk_dwrite(mmc_get_blk_desc(mmc), i, NUM_TEST_BLOCKS, data);
		if (ret != NUM_TEST_BLOCKS) {
	                snprintf(feedback, feedback_size,
			         "mmc write failure on block %d/%d", i, endblock);
			break;
		}

		ret = blk_dread(mmc_get_blk_desc(mmc), i, NUM_TEST_BLOCKS, verify);
		if (ret != NUM_TEST_BLOCKS) {
	                snprintf(feedback, feedback_size,
			         "mmc read failure on block %d/%d", i, endblock);
			break;
		}

		if (memcmp(data, verify, MMC_MAX_BLOCK_LEN*NUM_TEST_BLOCKS)) {
	                snprintf(feedback, feedback_size,
			         "data integrity failure on block %d/%d", i, endblock);
			break;;
		}
		if (i % PRINT_INTERVAL == 0) {
			printf("  tested %d/%ld (%d%%)\n", i,
                               (unsigned long)(mmc->capacity_user / MMC_MAX_BLOCK_LEN),
                               (unsigned int)((100*(unsigned long)i)/((mmc->capacity_user / MMC_MAX_BLOCK_LEN))));
		}
	}
...
int mmc_speed_test(struct mmc *mmc, unsigned int hz, char *feedback, int feedback_size)
{
	int i, ret;
	uint8_t data[MMC_MAX_BLOCK_LEN], verify[MMC_MAX_BLOCK_LEN];
	uint8_t value = 0xA5;
	...
	if (mmc->clock != hz)
	{
		printf("Expected %d hz; Got %d hz\n", hz, mmc->clock);
		snprintf(feedback, feedback_size, "Invalid Speed");
		goto respond;
	}

	ret = blk_dwrite(mmc_get_blk_desc(mmc), SPEED_TEST_BEGIN, 1, data);
	if (ret != 1) {
		snprintf(feedback, feedback_size, "mmc write failure");
		goto respond;
	}
	else
	{
		ret = blk_dread(mmc_get_blk_desc(mmc), SPEED_TEST_BEGIN, 1, verify);
		if (ret != 1) {
			snprintf(feedback, feedback_size, "mmc read failure");
			goto respond;
		}
		else
		{
			if (memcmp(data, verify, MMC_MAX_BLOCK_LEN)) {
				snprintf(feedback, feedback_size, "data integrity failure");
				for (i = 0; i < MMC_MAX_BLOCK_LEN; i++) {
                                	if (data[i] != verify[i]) {
						print_diffs(i, data[i], verify[i]);
					}
				}
			}
		}
...
respond:
	if (strlen(feedback)) {
		...
		snprintf(feedback, feedback_size, "Pass test at %d Khz", hz/1000);
		ret = 0;
	}
	printf("%s\n", feedback);
	return ret;
