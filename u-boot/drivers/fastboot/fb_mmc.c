			      ...
			      u32 download_bytes, char *response)
{
	...
	enum mmc_write_protect status;
	if (mmc_get_block_wp_status(find_mmc_device(CONFIG_FASTBOOT_FLASH_MMC_DEV),
				    info.start, &status)) {
		pr_err("cannot find verify WP status: '%s'\n", cmd);
		fastboot_fail("WP verify error", response);
		return;
	}
	if (MMC_WP_NONE != status) {
		pr_err("Block %ld Write protect state %d\n", info.start, status);
		fastboot_fail("Partition write protected", response);
		return;
	}
#endif
...
void fastboot_mmc_erase(const char *cmd, char *response)
{
	...
	if (!strncmp(cmd,"all", strlen("all"))){
		info.start = 0;
		info.blksz = 512;
		info.size = mmc->capacity / info.blksz;
	} else {
...
/**
 * fastboot_mmc_speed_test() - Test eMMC speed for fastboot
 *
 * @cmd: String with frequency to test in hz
 * @response: Pointer to fastboot response buffer
 */
void fastboot_mmc_speed_test(char *cmd, char *response)
{
	...
	/* strip off first word */
        strsep(&cmd, " ");
	hz = simple_strtoul(cmd, NULL, 10);
	if (hz)
	{
		struct mmc *mmc = find_mmc_device(CONFIG_FASTBOOT_FLASH_MMC_DEV);
		if (!mmc)
			fastboot_fail("No MMC device found", response);
		else
		{
			if (mmc_speed_test(mmc, hz, buf, RESPONSE_BUF_SIZE))
			{
	...
	{
		fastboot_fail("Invalid frequency", response);
	}
}

/**
 * fastboot_mmc_block_test() - Test eMMC write/read
 *
 * @cmd: No parameters expected
 * @response: Pointer to fastboot response buffer
 */
void fastboot_mmc_block_test(char *cmd, char *response)
{
	...
	{
		if (mmc_block_test(mmc, buf, RESPONSE_BUF_SIZE))
		{
			fastboot_fail(buf, response);
		} else {
			fastboot_okay(buf, response);
...
#endif //# CONFIG_IS_ENABLED(FASTBOOT_CMD_OEM_TESTMMC)
