...
int atsha204a_get_counter(struct udevice *dev, u32 *value, u8 counternum)
{
	int res, retry = ATSHA204A_TRANSACTION_RETRY;
	struct atsha204a_req req;
	struct atsha204a_resp resp;

	if ((NULL == dev) || (NULL == value) || (counternum > 1))
		return -EINVAL;

	req.function = ATSHA204A_FUNC_COMMAND;
	req.length = 7;
	req.command = ATSHA204A_CMD_COUNTER;

	req.param1 = 0; /* read */
	req.param2 = counternum;
	...
	do {
		...
		debug("ATSHA204A get counter retry (%d)\n", retry);
		retry--;
		atsha204a_wakeup(dev);
	} while (retry >= 0);

	memcpy(value, ((u8 *) &resp) + 1, sizeof(value));

	return res;
}

int atsha204a_inc_counter(struct udevice *dev, u32 *newvalue, u8 counternum)
{
	int res, retry = ATSHA204A_TRANSACTION_RETRY;
	struct atsha204a_req req;
	struct atsha204a_resp resp;

	if ((NULL == dev) || (NULL == newvalue) || (counternum > 1))
		return -EINVAL;
	...
	do {
		...
		debug("ATSHA204A increment counter retry (%d)\n", retry);
		retry--;
		atsha204a_wakeup(dev);
	} while (retry >= 0);

	memcpy(newvalue, ((u8 *) &resp) + 1, sizeof(newvalue));
