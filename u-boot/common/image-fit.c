...
__weak void *board_fdt_load_keys(void)
{
	return NULL;
}
...
#ifndef USE_HOSTCC
	if (NULL == gd_fdt_blob()) {
		void *blob = board_fdt_load_keys();
		if (blob) {
			gd_set_fdt_blob(blob);
			printf("No FDT found; loaded from board file\n");
		} else {
			printf("No FDT found for public key\n");
			goto error;
		}
...
#if defined(CONFIG_FIT_SECVER)
__weak int board_get_secver(uint32_t *board_secver)
{
	return -1;
}

__weak int board_set_secver(uint32_t secver)
{
	return -1;
}

__weak int board_verify_secver(uint32_t fit_secver, uint32_t board_secver)
{
	return -1;
}

int fit_get_secver(const void *fit, uint32_t *secver)
{
	const u32 *node_secver;
	int ndepth=0;
	int images_noffset, noffset, len; 
	uint8_t type;
	...
	for (noffset = fdt_next_node(fit, images_noffset, &ndepth);
		(noffset >= 0) && (ndepth > 0);
		noffset = fdt_next_node(fit, noffset, &ndepth)) {
		fit_image_get_type(fit, noffset, &type);
		if ((ndepth == 1) && (IH_TYPE_SECURITYVER == type)) {
			node_secver = fdt_getprop(fit, noffset, "data", &len);
			if (node_secver && (len == sizeof(*node_secver))) {
				*secver = fdt32_to_cpu(*node_secver);
				return 0;
			}
		}
	}
...
/**
 * fit_verify_secver - verify that the fit image security
 *                     version is >= system security version
 * @fit: pointer to the FIT format image header
 *
 * returns:
 *     1, if security version check passes
 *     0, otherwise (or on error)
 */
int fit_verify_secver(const void *fit)
{
	uint32_t fit_secver, system_secver;

	if (fit_get_secver(fit, &fit_secver)) {
		return 0;
	}

	if (board_get_secver(&system_secver)) {
		return 0;
	}

	if (board_verify_secver(fit_secver, system_secver)) {
		return 0;
	}

	printf("image secver: %d  system secver: %d\n",
		fit_secver, system_secver);

	if (system_secver > fit_secver) {
		return 0;
	}

	if (fit_secver > system_secver) {
		/* Update system secver if FIT is higher */
		if (board_set_secver(fit_secver)) {
			return 0;
		}
		printf("system secver updated: %d\n", fit_secver);
	}
