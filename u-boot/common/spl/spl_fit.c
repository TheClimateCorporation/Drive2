			...
			struct spl_load_info *info, ulong sector, void *fit)
{
	...
	unsigned long count;
	struct spl_image_info image_info;
	struct spl_image_info secver_info;
	int node = -1;
	int images, ret;
	...
	/* Locate and load/authenticate security version */
	
	node = spl_fit_get_image_node(fit, images, FIT_SECVER_PROP, 0);

	if (node < 0) {
		printf("Error: missing secver node\n");
		return -ENOENT;
	}

	/* verify the secver node and exit if authentication fails. secver_info won't be used after this */

	ret  = spl_load_fit_image(info, sector, fit, base_offset, node, &secver_info);
	if (ret) {
	 	return ret;
	}
	 ...
	 * We do NOT allow this to fail because of secure boot requirements
	 */
	if (spl_image->os == IH_OS_U_BOOT) {

		/* Load FDT */
		ret = spl_fit_append_fdt(spl_image, info, sector, fit, images, base_offset);
		if (ret) {
			return ret;
		}
	}
