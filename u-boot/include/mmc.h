...
#define MMC_CMD_CLEAR_WRITE_PROT	29
...
#define EXT_CSD_US_PERM_WP_DIS		(0x10)
#define EXT_CSD_US_PWR_WP_DIS		(0x08)
#define EXT_CSD_US_PERM_WP_EN		(0x04)
#define EXT_CSD_US_PWR_WP_EN		(0x01)

enum mmc_write_protect {
	/* values have meaning defined in CMD31 */
	MMC_WP_NONE 		= 0x0,
	MMC_WP_TEMPORARY	= 0x1,
	MMC_WP_POWERON		= 0x2,
	MMC_WP_PERMANENT	= 0x3,
};
#endif
...
#if CONFIG_IS_ENABLED(MMC_WRITE_PROTECT)
int mmc_get_block_wp_status(struct mmc *mmc, lbaint_t addr,
			    enum mmc_write_protect *status);

int mmc_set_block_wp_status(struct mmc *mmc, lbaint_t addr,
			    lbaint_t blocks,
			    enum mmc_write_protect status);
#endif
...
int mmc_speed_test(struct mmc *mcc, unsigned int hz, char *feedback, int feedback_size);
int mmc_block_test(struct mmc *mcc, char *feedback, int feedback_size);
#endif /* _MMC_H_ */
