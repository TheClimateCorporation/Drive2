...
enum {
	...
	FASTBOOT_COMMAND_CONTINUE,
        /* For similar starting commands (like reboot & reboot-bootloader)
         * the longer command needs to be listed first so the matching
         * happens correctly
         */
...
/**
 * fastboot_oem_handler() - handle fastboot oem commands
 *                           
 * @cmd_string: Pointer to command string
 * @response: Pointer to fastboot response buffer
 *
 * Return: 0 if command was handled (OKAY or FAIL).
 *         -1 if command was not handled.
 * This function should be overridden in your board file with one
 * which handles any board specific fastboot oem commands.
 */
int fastboot_oem_handler(char *cmd_parameter, char *response);
